		...
		if not shoot.is_empty():
			if is_coord_on_line(shoot, hovered_tile):
				if prev_bomb_tile2 != null and prev_bomb_tile2 in all_bomb_coords:
					...
					prev_bomb_tile2 = null
				if shoot_tile != null:
					if shoot_tile in all_bomb_coords:
						highlight_layer.set_cell(shoot_tile,0, highlight_bomb_coords)
					elif shoot_tile in projected_bomb_coords:
						highlight_layer.set_cell(shoot_tile, 0, highlight_bomb_projection_coords)
					else:
...
func parse_linear_exp_string(expression : String) -> Dictionary:
	var m : float
	var b : float
	#print("expression in parlse_linear_exp_string: ",expression)
	if 'x' in expression:
...
func get_intersect_point(expression1: String, expression2 : String) -> Variant:
	...
	#print("laser expression: ", eq1)
	var eq2 = parse_linear_exp_string(expression2)
	#print("move vector: ", eq2)
	var m1 = eq1["m"]
	var m2 = eq2["m"]
	var b1 = eq1["b"]
	var b2 = eq2["b"]
...
func segment_crosses_line(start: Vector2, end: Vector2, m: float, b: float) -> bool:
	...
	var t = (m * start.x + b - start.y) / denom
	return t >= 0.0 and t <= 1.0
